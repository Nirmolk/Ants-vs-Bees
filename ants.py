...
class ShortThrower(ThrowerAnt):
    ...
    max_range=3
    min_range=0
    food_cost=2 # Change to True to view in the GUI
    # END Problem 4
    ...
    def action(self, colony):
        ...
        if self.imposter: 
            ...
            while current_place.exit is not None: 
                ...
                if current_place.ant:
                    if current_place.ant not in self.ants_lst:
                        self.ants_lst.append(current_place.ant)
                        current_place.ant.damage *=2
                    if current_place.ant.is_container:
                        c_ant = current_place.ant.contained_ant
                        if c_ant and c_ant not in self.ants_lst and hasattr(c_ant, 'damage'):
                            self.ants_lst.append(current_place.ant.contained_ant)
      ...
      #  def ddamage(place): 
       #     if not type(place.ant)==QueenAnt: 
        #        if place.ant is not None and not place.ant.double:
         #           place.ant.double=True
          #          place.ant.damage *=2
           #         if place.ant.container:
            #            if not type(place.ant.ant.ant)==QueenAnt:
             #               place.ant.ant.double=True
            ...
            #behind= self.place
            #while self.exit is not None: 
             #   beind=behind.exit
              #  ddamage(behind)

            #infront=self.place
            #   infront=infront.entrance
             #   ddamage(infront)
